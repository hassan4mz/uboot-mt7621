...
static void upload_handler2(enum httpd_uri_handler_status status,
	struct httpd_request *request,
	struct httpd_response *response)
{
	char *buff, *md5_ptr, *size_ptr, size_str[16];
	...
	if (status == HTTP_CB_NEW) {
		...
		/* TODO: add factory validation here if necessary */

		if (output_plain_file(response, "upload2.html")) {
			response->info.code = 500;
			return;
		}
		...
		upload_data_id = upload_id;
		upload_data = factory->data;
		upload_size = factory->size;

		return;
	}

	if (status == HTTP_CB_CLOSED) {
		file = fs_find_file("upload2.html");
...
static void result_handler2(enum httpd_uri_handler_status status,
	struct httpd_request *request,
	struct httpd_response *response)
{
	const struct fs_desc *file;
